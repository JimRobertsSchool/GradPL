...
static int test_dim_rotate[] = {64, 128, 256, 512, 1536};
static int test_dim_smooth[] = {32, 64, 128, 256, 768};
...
static int baseline_min(int a, int b) { return (a < b ? a : b); }
static int baseline_max(int a, int b) { return (a > b ? a : b); }

static void baseline_initialize_pixel_sum(baseline_pixel_sum *sum) 
{
	sum->red = sum->green = sum->blue = 0;
	sum->num = 0;
	return;
}
static void baseline_accumulate_sum(baseline_pixel_sum *sum, pixel p) 
{
	sum->red += (int) p.red;
...
static void baseline_assign_sum_to_pixel(pixel *current_pixel, baseline_pixel_sum sum) 
{
...
static pixel baseline_avg(int dim, int i, int j, pixel *src) 
{
	int ii, jj;
	baseline_pixel_sum sum;
	pixel current_pixel;

	baseline_initialize_pixel_sum(&sum);
	for(ii = baseline_max(i-1, 0); ii <= baseline_min(i+1, dim-1); ii++) 
		for(jj = baseline_max(j-1, 0); jj <= baseline_min(j+1, dim-1); jj++) 
			baseline_accumulate_sum(&sum, src[RIDX(ii, jj, dim)]);

	baseline_assign_sum_to_pixel(&current_pixel, sum);
...
void baseline_naive_smooth(int dim, pixel *src, pixel *dst) 
{
	...
	for (i = 0; i < dim; i++)
		for (j = 0; j < dim; j++)
			dst[RIDX(i, j, dim)] = baseline_avg(dim, i, j, src);
}
